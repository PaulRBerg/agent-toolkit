...
are cleared or replaced. Ignored local directories whose parents exist in the prepared tree are projected into the
...
and leaves the transaction prepared for retry. Repositories without `[validation]` retain the existing hook, signing,
