...
pub(super) fn triage_prompt(
    ...
    let authorized = serde_json::to_string_pretty(authorized_paths)?;
    Ok(format!(
        "You are the autonomous ai-coord findings triager. Work only on the supplied findings.\n\
         You are in an isolated worktree on branch triage/{run_id}. Never touch the original checkout.\n\
         ...
         The worker already owns the exact safe-document paths listed below. Do not run ai-coord lifecycle or status \
         commands. Edit no other tracked path. Run narrow validation. For each canonical finding you fix, create exactly \
         one local commit on triage/{run_id} through ai-commit with a `Finding-ID: <ID>` trailer. Never push.\n\
         For everything outside the safe tier, explicitly invoke `$task-handoff` and publish the exact repository-relative \
         `.ai/task-handoffs/FINDING_<UPPERCASE_ID>.md` with its no-clipboard workflow, inside this worktree. Preserve the ledger ID's \
